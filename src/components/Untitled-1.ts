...
export default function HomeCliente() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, tipo: 'portada' | 'avatar' | 'producto') => {
    ...
    try {
      ...
      if (tipo === 'portada') {
        const nuevaInfoLocal = { ...infoLocal, portadaUrl: urlNube };
        setInfoLocal(nuevaInfoLocal);
        await saveShopConfigData({ infoLocal: nuevaInfoLocal, categorias, productos });
      }

      if (tipo === 'avatar') {
        const nuevaInfoLocal = { ...infoLocal, avatarUrl: urlNube };
        setInfoLocal(nuevaInfoLocal);
        await saveShopConfigData({ infoLocal: nuevaInfoLocal, categorias, productos });
      }
        ...
        <div className="p-4">
          ...
          {carrito.length === 0 ? (
            ...
            <form onSubmit={enviarPedidoWhatsApp} className="space-y-5">
              ...
              <div className="bg-neutral-900/60 p-4 rounded-2xl space-y-4 border border-neutral-700/30">

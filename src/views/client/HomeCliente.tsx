...
export default function HomeCliente() {
  ...
  // Procesador de archivos de galería del teléfono a Base64
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, tipo: 'portada' | 'avatar' | 'producto') => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onloadend = () => {
      const base64String = reader.result as string;
      if (tipo === 'portada') setInfoLocal(p => ({ ...p, portadaUrl: base64String }));
      if (tipo === 'avatar') setInfoLocal(p => ({ ...p, avatarUrl: base64String }));
      if (tipo === 'producto') setProdForm(p => ({ ...p, imagen: base64String }));
    };
  ...
  return (
    <div className="max-w-md mx-auto bg-neutral-900 min-h-screen pb-24 shadow-2xl relative font-sans text-white border-x border-neutral-800">
        ...
        <div className="p-4">
          ...
          {carrito.length === 0 ? (
            ...
            <form onSubmit={enviarPedidoWhatsApp} className="space-y-5">
              ...
              <button type="submit" className="w-full bg-emerald-500 text-neutral-950 font-black py-4 rounded-2xl flex items-center justify-center gap-2 text-base shadow-lg"><MessageSquare size={18} fill="currentColor" />Enviar Pedido por WhatsApp</button>
            </form>
          )}
        </div>
      )}
        ...
        <div className="p-4 space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button 
              ...
              className="bg-neutral-800 hover:bg-neutral-700 text-white font-bold py-2 px-3 rounded-xl text-xs transition"
            >
              Cerrar Panel
            </button>
          </div>

          {/* MÓDULO CONTROL DE CAJA Y TURNOS */}
          ...
          {/* MÓDULO REDES SOCIALES Y DATOS BANCARIOS */}
          <div className="bg-neutral-800/60 p-4 rounded-2xl border border-neutral-700/30 space-y-3">
          ...
          {/* MÓDULO CONTROL DE SECCIONES / CATEGORÍAS */}
          <div className="bg-neutral-800/60 p-4 rounded-2xl border border-neutral-700/30 space-y-3">
          ...
          {/* MÓDULO LISTADO Y ABM DE COMIDAS (CON LÁPIZ Y TACHO) */}
          <div className="bg-neutral-800/60 p-4 rounded-2xl border border-neutral-700/30 space-y-3">
            ...
            {/* Formulario rápido para Añadir/Editar */}
            <div className="bg-neutral-900 p-3 rounded-xl border border-neutral-700/50 space-y-2">
              ...
              <button 
                type="button" 
                onClick={() => {
                  if(!prodForm.nombre || !prodForm.precio) return;
                ...
                className="w-full bg-gradient-to-r from-yellow-400 to-yellow-500 text-neutral-950 font-black py-2 rounded-lg text-xs uppercase"
              >
                {editandoProductoId ? "Guardar Cambios" : "Añadir al Menú"}
              </button>
            </div>

            {/* Lista con botones de acción rápida */}
      ...
      {/* NAV INFERIOR LIMPIO (SÓLO MENÚ Y CARRITO) */}
      ...
      {/* ACCESO ADMIN CAMUFLADO: Barrita 100% invisible abajo de todo el contenedor */}
      <button 
        onClick={() => {
          const password = prompt("Ingresá la contraseña del administrador:");
          if (password === "applodefiore") { // <--- Cambiá "applodefiore" por tu clave
